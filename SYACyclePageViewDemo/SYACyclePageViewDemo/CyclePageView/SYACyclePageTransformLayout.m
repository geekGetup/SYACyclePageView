...
typedef NS_ENUM(NSUInteger, SYATransformLayoutItemDirection) {
    SYATransformLayoutItemLeft,
    SYATransformLayoutItemCenter,
    SYATransformLayoutItemRight,
};
@interface SYACyclePageTransformLayout(){
    struct {
...
@interface SYACyclePageViewLayout()

@property (nonatomic, weak) UIView *pageView;

@end

@implementation SYACyclePageTransformLayout
- (instancetype)init {
    if (self = [super init]) {
...
- (void)setDelegate:(id<SYACyclePageTransformLayoutDelegate>)delegate {
    _delegate = delegate;
    _delegateFlags.initializeTransformAttributes = [delegate respondsToSelector:@selector(pageViewTransformLayout:initializeTransformAttributes:)];
    _delegateFlags.applyTransformToAttributes = [delegate respondsToSelector:@selector(pageViewTransformLayout:applyTransformToAttributes:)];
}

- (void)setLayout:(SYACyclePageViewLayout *)layout {
    _layout = layout;
...
- (SYATransformLayoutItemDirection)directionWithCenterX:(CGFloat)centerX {
    SYATransformLayoutItemDirection direction= SYATransformLayoutItemRight;
    ...
    if (ABS(centerX - contentCenterX) < 0.5) {
        direction = SYATransformLayoutItemCenter;
    }else if (centerX - contentCenterX < 0) {
        direction = SYATransformLayoutItemLeft;
    }
    return direction;
}
...
{
    return _layout.layoutType == SYACyclePageTransformLayoutNormal ? [super shouldInvalidateLayoutForBoundsChange:newBounds] : YES;
}

- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    if (_delegateFlags.applyTransformToAttributes || _layout.layoutType != SYACyclePageTransformLayoutNormal) {
        ...
        for (UICollectionViewLayoutAttributes *attributes in attributesArray) {
            ...
            if (_delegateFlags.applyTransformToAttributes) {
                [_delegate pageViewTransformLayout:self applyTransformToAttributes:attributes];
            }else {
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_delegateFlags.initializeTransformAttributes) {
        [_delegate pageViewTransformLayout:self initializeTransformAttributes:attributes];
    }else if(_layout.layoutType != SYACyclePageTransformLayoutNormal){
...
- (void)initializeTransformAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(SYACyclePageTransformLayoutType)layoutType {
    switch (layoutType) {
        case SYAyclePageTransformLayoutLinear:
            [self applyLinearTransformToAttributes:attributes scale:_layout.minimumScale alpha:_layout.minimumAlpha];
            break;
        case SYACyclePageTransformLayoutCoverflow:
        {
...
- (void)applyTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(SYACyclePageTransformLayoutType)layoutType {
    switch (layoutType) {
        case SYAyclePageTransformLayoutLinear:
...
- (void)applyLinearTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes scale:(CGFloat)scale alpha:(CGFloat)alpha {
    ...
    if (_layout.adjustSpacingWhenScroling) {
        SYATransformLayoutItemDirection direction = [self directionWithCenterX:attributes.center.x];
        CGFloat translate = 0;
        switch (direction) {
            case SYATransformLayoutItemLeft:
                translate = 1.15 * attributes.size.width*(1-scale)/2;
                break;
            case SYATransformLayoutItemRight:
                translate = -1.15 * attributes.size.width*(1-scale)/2;
                break;
            default:
                // center
...
@implementation SYACyclePageViewLayout
